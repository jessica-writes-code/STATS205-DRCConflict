...
B <- 5000
bootstrap.auc <- function(event.type, est.pi, est.lscv, est.lscv.RN, est.bcv1, est.ns) {
...
## Function to do ttests on AUCs
t.auc <- function(df) {
  val <- rep(NA,10)
  val[1] <- t.test(df$auc.pi,df$auc.lscv)$p.value
  val[2] <- t.test(df$auc.pi,df$auc.lscv.RN)$p.value
  val[3] <- t.test(df$auc.pi,df$auc.bcv1)$p.value
  val[4] <- t.test(df$auc.pi,df$auc.ns)$p.value
  
  val[5] <- t.test(df$auc.lscv,df$auc.lscv.RN)$p.value
  val[6] <- t.test(df$auc.lscv,df$auc.bcv1)$p.value
  val[7] <- t.test(df$auc.lscv,df$auc.ns)$p.value
  
  val[8] <- t.test(df$auc.lscv.RN,df$auc.bcv1)$p.value
  val[9] <- t.test(df$auc.lscv.RN,df$auc.ns)$p.value
  
  val[10] <- t.test(df$auc.bcv1,df$auc.ns)$p.value
  return(val)
}
...
# Bootstrap CIs
bs.r <- bootstrap.auc(event.type="R", estimates.r.pi, estimates.r.lscv, estimates.r.lscv.RN, estimates.r.bcv1, estimates.r.ns)
...
# Bootstrap CIs
bs.v <- bootstrap.auc(event.type="V", estimates.v.pi, estimates.v.lscv, estimates.v.lscv.RN, estimates.v.bcv1, estimates.v.ns)
